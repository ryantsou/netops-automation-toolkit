...
# Required tools
sudo apt-get update
sudo apt-get install -y bash curl wget net-tools dnsutils iproute2 iputils-ping nmap bc openssl coreutils
```
...
| Script | Description | Parameters |
|--------|-------------|------------|
| `network-report.sh` | Complete network report | `-o` output dir, `-f` format (`text`) |
...
- `config/switches.txt`: Switch targets for VLAN provisioning

Files generated by the installer (`./install.sh`):
...
```bash
# Lint scripts when ShellCheck is available
